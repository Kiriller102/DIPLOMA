...
class Kuznyechik {
public:
    explicit Kuznyechik(const short *master_key);

    void Encrypt(short *a);
    void Decrypt(short *a);
...
private:
    ...
    short Linear(short *a);

    void LSX(uint64_t k1, uint64_t k2);

    void F(const short *k, short *a_1, short *a_0);
    void X(short *a, const short *k);
    void L(short *a);

    void SmallLSX(short *a, const short *k);

    void LS(short *a);

    short GaluaMult(short x, short y);

    static constexpr int kGaluaPoly = 451;
    short temp_a[kBlockSize * 2];
    short c_coefs[kBlockSize * 2][kBlockSize];
    short k_coefs[10][kBlockSize];
    uint64_t big_k_coefs[10][2];
    short powers[256];
    short inv_powers[256];
    short l_temp_a[kBlockSize * 2];
    uint64_t precalc[256][kBlockSize][2];
    short linear_coefs[kBlockSize] = {148, 32, 133, 16, 194, 192, 1, 251, 1, 192, 194, 16, 133, 32, 148, 1};
    short inv_permutation[256];
    short permutation[256] = {252, 238, 221, 17, 207, 110, 49, 22, 251, 196, 250, 218, 35, 197, 4, 77, 233,
